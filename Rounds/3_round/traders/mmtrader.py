...
class MMTrader(TraderBase):
    def __init__(self, name, state, new_trader_data):
        super().__init__(name, state, new_trader_data)
        
        # Load historical data using the helper method
        self.history_key = f"{self.name}_history"
        self.price_history  = self.last_traderData.get(self.history_key, [])
    ...
    def get_orders(self):
        ...
        s = self.wall_mid
        q = self.expected_position
        
        # Update history with current mid price
        self.price_history.append(s)
        self.price_history = self.price_history[-(lookback + 1):]
        
        # Compute variance on returns
        if len(self.price_history) > 2: # Need at least 3 prices to get 2 returns for ddof=1
        ...
        delta = gamma * var + (2 / gamma) * math.log(1 + gamma / k)

        as_bid = math.floor(r - delta / 2)
        as_ask = math.ceil(r + delta / 2)

        new_bid_price = min(as_bid, self.best_bid+1)
        new_ask_price = max(as_ask, self.best_ask-1)

        self.bid(self.best_bid+1, self.max_allowed_buy_volume)
        self.ask(self.best_ask-1, self.max_allowed_sell_volume)
        
        self.new_trader_data[self.history_key] = list(self.price_history)
