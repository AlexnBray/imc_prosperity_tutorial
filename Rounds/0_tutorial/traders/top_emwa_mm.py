...
class Trader:
    ...
    def trade_tomatoes(self, order_depth: OrderDepth, position: int, prev_fv: float) -> tuple[List[Order], float]:
        ...
        limit = 80
        soft_limit = 72
        min_edge = 2 # Wider spread edge to combat negative-edge fills
    ...
    def run(self, state: TradingState) -> tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "EMERALDS":
                ...
                result[product], current_fv = self.trade_tomatoes(order_depth, position, prev_fv)
                data[product] = current_fv 
