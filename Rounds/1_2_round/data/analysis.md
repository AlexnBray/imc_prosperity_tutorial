...
For each bot / behavior:
    ...
    Identify the primary variable (usually price placement relative to FV)
    Condition EVERY other variable on the primary variable
    Look for splits: does the distribution change across conditions?
    If yes, that's a structural feature of the bot's algorithm — model it
    If no, it's truly independent — model it as independent
    Never declare a model "done" until every pairwise conditional has been checked
