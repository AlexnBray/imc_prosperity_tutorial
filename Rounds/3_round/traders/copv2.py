...
COINT_BETA = 1.0 # REPLACE WITH YOUR NOTEBOOK OLS/COINTEGRATION FIT VALUES
COINT_ALPHA = 0.0 # """"""""""
...
class HydrogelPack:
    ...
    def get_orders(self):
        ...
        if self.wall_mid is not None:
            ...
            for buy_p, buy_v in self.mkt_buy_orders.items():
                overbid_price = buy_p + 1

                if buy_v > 1 and overbid_price < self.wall_mid:
                    break
                elif buy_p < self.wall_mid:
                    bid_price = max(bid_price, buy_p)
                    break

            for sell_p, sell_v in self.mkt_sell_orders.items():
                underbid_price = sell_p -1
                if sell_v > 1 and underbid_price > self.wall_mid:
                    ask_price = min(ask_price, )
