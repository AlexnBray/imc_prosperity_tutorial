...
def price_graph(df, trades_json, product, timestamp):
    ...
    df_product["mm_mid"] = df_product["mm_mid"].ffill()
    fig = px.line(
...
def get_df_orderbook(df, timestamp):
    df_ob = df.copy()
    # 1. Set the index properly (using drop=False ensures 'timestamp' remains a column if needed)
    df_ob = df_ob.set_index("timestamp")

    # 2. Check if the timestamp exists to prevent a KeyError
    if timestamp in df_ob.index:
        data = df_ob.loc[timestamp]
    else:
        # 3. Fallback: Return an empty Series/DataFrame or the closest previous tick
        # Option A: Return a blank row (stops the crash)
        return pd.DataFrame()
    min_bid = (
